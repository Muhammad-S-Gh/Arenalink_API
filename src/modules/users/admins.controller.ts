...
export class AdminsController {
    ...
    @ApiBody({
        schema: {
            ...
            properties: {
                name: {
                    type: 'string',
                    example: '{"en":"Football","ar":"كرة القدم"}', // comes as string in form-data
                },
                description: {
                    type: 'string',
    ...
    async createBasic(@Body() dto: any, @UploadedFile() icon: Express.Multer.File) {
        try {
            // ✅ Parse name & description if they are JSON strings
    ...
    // get facility approved reservations
