...
export class PaymentService {
    ...
    async createPaymentIntent(reservationId: number, user: User) {
        ...
        try {
            const res = await this.dataSource.transaction(async (manager) => {
                const isoDate = reservation.date; // yyyy-mm-dd
                const dateInt = parseInt(String(isoDate).replace(/-/g, ''), 10); // e.g. 20250825
                const slotId = reservation.slot?.id ?? null;
                const lockKey1 = slotId ?? reservation.facility.id; // numeric
                const lockKey2 = dateInt;

                // advisory transactional lock (blocks concurrent transactions trying same key)
                await manager.query('SELECT pg_advisory_xact_lock($1, $2)', [lockKey1, lockKey2]);

                // this block might be redundant since i always mark the payment as cancelled if it didn't succeed and
                // If a payment row was already saved for this Stripe intent (retry), return it (like this i might allow to retry cancelled intents right ?? isn't that bad)
                ...
                // Optionally: if there's already a PENDING payment for this reservation, return it. (in case something happened and the intent stayed incomplete this will retry for the pending existing payment intent )
                ...
                // 1) confirmed reservations for this slot on the date
                ...
                // 2) payments already holding this slot/date (PENDING payments)
                ...
                // 3) legacy overlapping confirmed reservations count (if you want to include these)
                ...
                // 4) determine capacity for this slot (slot-level capacity may be pulled from facility/category)
                // — reuse your facility capacity extraction (you may load facility inside manager or reuse preloaded)
                // For simplicity here we assume capacity is per-slot found via facility.category attributes:
    ...
    // async savePayment(user: User, paymentIntent: Stripe.PaymentIntent, reservation: Reservation): Promise<Payment> {
    //     const payload: DeepPartial<Payment> = {
    //         user,
    //         reservation,
    //         stripeClientSecret: String(paymentIntent.client_secret),
    //         status: PaymentStatus.PENDING,
    //         amountCents: Number(reservation.priceCents),
    //         currency: paymentIntent.currency ?? this.configService.getOrThrow('STRIPE_CURRENCY'),
    //         stripePaymentIntentId: paymentIntent.id,
    //         facility: reservation.facility,
    //         description: {
    //             ar: `المبلغ المستحق للحجز رقم ${reservation.id} بمجموع ${reservation.price} للمستخدم السيد ${user.firstName} ${user.lastName} (معرف المستخدم: ${user.id}) بتاريخ  ${new Date()}`,
    //             en: `Payment due for reservation ${reservation.id} totaling ${reservation.price} for user ${user.id} Mr. ${user.firstName} ${user.lastName} at ${new Date()} `,
    //         },
    //         title: {
    //             ar: `فاتورة للحجز رقم ${reservation.id}`,
    //             en: `Invoice for reservation ${reservation.id}`,
    ...
    //     const payment = this.paymentRepo.create(payload);
        ...
        try {
            ...
            if (event.type === 'payment_intent.succeeded') {
                ...
                await this.dataSource.transaction(async (manager) => {
                    ...
                    const isoDate = reservation.date; // yyyy-mm-dd
                    const dateInt = parseInt(isoDate.replace(/-/g, ''), 10); // 20250825
    ...
    // add refund if the payment is completed and more than today
    ...
    async userRefundPayment(paymentIntentId: string, user: User) {
        ...
        try {
            ...
            // update payment record (transaction optional)
